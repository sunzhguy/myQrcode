...
#include "video.h"
#include "window.h"
#include "timer.h" //add by sunzhguy  to gettime now
                                         ...
                                         unsigned height)
{
    ...
    dst->width = width;
    dst->height = height;
	sunprintf(19,"sunzhguy dst[,%d,%d],src[%d,%d,%d,%d]\n",dst->width,dst->height,src->crop_x, src->crop_y,src->crop_w, src->crop_h);
	sunprintf(19,"xxxxxxxxxxxxxxxxxx%.4s,%.4s \n",(char*)&(src->format),(char*)&fmt);
	...
	long t1=_zbar_timer_now ();

    srcfmt = _zbar_format_lookup(src->format);
    dstfmt = _zbar_format_lookup(dst->format);
	long t2=_zbar_timer_now ();

	
	sunprintf(19,"xxxxxxxxxxxxxxxxxx%08x,%08x time=%d \n",src->format,dst->format,t2-t1);
	sunprintf(19,"xxxxxxxxxxxxxxxxxx%.4s,%.4s \n",(char*)&(src->format),(char*)&(dst->format));
    if(!srcfmt || !dstfmt)
        /* FIXME free dst */
        return(NULL);
	sunprintf(19,"xxxxxxxxxxxxxxxxxx%d,%d ||%d,%d\n",srcfmt->group,dstfmt->group,srcfmt->p.cmp,dstfmt->p.cmp);
	...
	t2=_zbar_timer_now ();
	 sunprintf(19,"sunzhguy get the conversion,%d,%d,tm=%d\n",srcfmt->group,dstfmt->group,t2-t1);
    if(!dst->data) {
        /* conversion failed */
                           ...
                           zbar_window_t *win)
{
    ...
    for(fmt = _zbar_formats; *fmt; fmt++) {
        ...
        zprintf(4, "%.4s(%08" PRIx32 ") -> %.4s(%08" PRIx32 ") (%x)\n",
                (char*)fmt, *fmt, (char*)&win_fmt, win_fmt, cost);

		zprintf(4, "sunzhguy ---%.4s(%08" PRIx32 ") xxxxxxxxxxxx %.4s (%x)/(%x)\n",
                    (char*)fmt, *fmt, (char*)&min_fmt, min_cost, cost);
        if(min_cost > cost/*&&cost==24*/) {
			printf("min_cost=%d,cost=%d\n",min_cost,cost);
            min_cost = cost;
            min_fmt = *fmt;
            if(!cost)
                break;
    ...
    if(!min_fmt && vdo->emu_formats) {
        ...
        */
        for(fmt = _zbar_formats; *fmt; fmt++) {
			...
			zprintf(4, "%.4s(%08" PRIx32 ") xxxxxxxxxxxx %.4s (%d)/(%d)\n",
                    (char*)fmt, *fmt, (char*)&min_fmt, min_cost, cost);
            if(min_cost > cost) {
                min_cost = cost;
                min_fmt = *fmt;
