...
#define RECYCLE_BUCKETS     5//---------orig5
...
inline void zbar_image_scanner_recycle_image (zbar_image_scanner_t *iscn,
                                              zbar_image_t *img)
{
    zbar_symbol_set_t *syms = iscn->syms;//zbar_image_scanner_szbar_scanner_s
    if(syms && syms->refcnt) {
        if(recycle_syms(iscn, syms)) {   //zbar_image_scanner_s
...
unsigned long sun_time=0;//sunzghuy
                              ...
                              zbar_symbol_t *sym)
{
    if(iscn->enable_cache) {
        ...
        age = sym->time - entry->time;//zbar_symbol_s
        entry->time = sym->time;
		sunprintf(8,"age==%d,entryTime=%lu,[%d,%d]\n",age,entry->time,CACHE_PROXIMITY,CACHE_HYSTERESIS);
		...
		//////////////////////////////
		sunprintf(8,"near_thresh==%d,far_thresh=%lu,dup=%d]\n",near_thresh,far_thresh,dup);
        if((!dup && !near_thresh) || far_thresh) {
            int type = sym->type;
            int h = _zbar_get_symbol_hash(type);
			
            entry->cache_count = -iscn->sym_configs[0][h];
			sunprintf(8,"entry->cache_count[%d,%d]\n",entry->cache_count,h);
        }
...
static void symbol_handler (zbar_decoder_t *dcode)
{
    ...
    unsigned datalen;
    zbar_symbol_t *sym;
	zprintf(96, "sunzhguy symbor type(%d)\n", type);
...
{
    zbar_image_scanner_t *iscn = calloc(1, sizeof(zbar_image_scanner_t));//zbar_image_scanner_s
    if(!iscn)
        return(NULL);
...
int zbar_image_scanner_set_config (zbar_image_scanner_t *iscn,
                                   ...
                                   int val)
{
   zprintf(5, "sscan config by sunzhguy sym=%d,cfg==%d,ean_config=%d,%d\n",sym,cfg, iscn->ean_config,val);
    if((sym == 0 || sym == ZBAR_COMPOSITE) && cfg == ZBAR_CFG_ENABLE) {
		
        iscn->ean_config = !!val; //zbar_image_scanner_s zbar_scanner_s 
...
int zbar_scan_image (zbar_image_scanner_t *iscn,
                     zbar_image_t *img)
{
    zbar_symbol_set_t *syms;//zbar_symbol_set_s
    const uint8_t *data;
    zbar_scanner_t *scn = iscn->scn;
...
#ifdef ENABLE_QRCODE
    _zbar_qr_reset(iscn->qr);//zbar_image_scanner_s
#endif
    ...
    zbar_image_scanner_recycle_image(iscn, img);//zbar_symbol_set_s 
    syms = iscn->syms;
    if(!syms) {
        syms = iscn->syms = _zbar_symbol_set_create();
        STAT(syms_new);
        zbar_symbol_set_ref(syms, 1);//zbar_symbol_set_s zbar_scanner_s zbar_image_scanner_s
    }
    else
    ...
    int nean = 0, naddon = 0;
    if(syms->nsyms) {
		zprintf(15, "sunzhguy........15.....syms->nsyms=%d\n",syms->nsyms);
        zbar_symbol_t **symp;
        for(symp = &syms->head; *symp; ) {
	        ...
	        if((sym->type == ZBAR_CODABAR || filter) && sym->quality < 4) {
                    if(iscn->enable_cache) {
							zprintf(15, "sunzhguy........15.....type=%d,[cachecount=%d]\n",sym->type,sym->cache_count);
                        /* revert cache update */
                        zbar_symbol_t *entry = cache_lookup(iscn, sym);
                        if(entry)
                            entry->cache_count--;
    ...
    if(syms->nsyms && iscn->handler)
    	{
    	 zprintf(5,"sunzhguy.................................syms->nsyms[==%d]......................................\n",syms->nsyms);
         iscn->handler(img, iscn->userdata);//zbar_image_scanner_s  zbar_image_data_handler_t
    	}
    svg_close();
	//Debug for sunzhguy
			unsigned long endtm=_zbar_timer_now();
