...
void sig_intr(int sig_no)
{ 
	printf("==>%s %s() L%d\n,sig_no==%d\n", __FILE__, __func__, __LINE__,sig_no);
     int rc = zbar_processor_user_wait(proc, 300);
	 printf("rc=%d\n",rc);
	if(proc)
...
static void data_handler (zbar_image_t *img, const void *userdata)
{
    ...
    assert(sym);
    int n = 0;
	printf("enter to hander...format=%d,%d\n",format,zbar_image_get_data_length(img));
    for(; sym; sym = zbar_symbol_next(sym)) {
		int ret=0;
        if((ret=zbar_symbol_get_count(sym)))
        	{
        	 printf("sunzhguy ret==%d",ret);
             continue;
        	}

        zbar_symbol_type_t type = zbar_symbol_get_type(sym);
		printf("type==[%d],ZBAR_PARTIAL==[%d],Quality=%d,codesize=%d",type,ZBAR_PARTIAL,zbar_symbol_get_quality(sym),zbar_symbol_get_data_length(sym));
...
int main (int argc, const char *argv[])
{
     ...
     */
    proc = zbar_processor_create(1);//zbar_processor_s
...
/////////////////////sunzhguy  the zbar is to process in main/////////////////////
    if(infmt || outfmt)
