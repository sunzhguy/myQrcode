...
# endif
            ...
            sunprintf(5,"sunzhguy event wait rc=%d,state=%d\n",rc,event->state);
        }
    }
                        ...
                        zbar_mutex_t *lock)
{
  ...
  printf("_zbar_thread_start start..............\n");
                               ...
                               int i)
{
	mydebugTm.cur_tm=_zbar_timer_now();
			...
			sunprintf(8, "xxxxxxxxxxxxxxxxxxxxxx by sunzhguy==%ld-----i==[%d]\n",tm,i);
    ...
    if(proc->streaming) {
		...
		mydebugTm.cur_tm=_zbar_timer_now();
		 unsigned long tm=(mydebugTm.cur_tm-mydebugTm.pre_tm);
		zprintf(20, "zbar get the.. stream--to procee by sunzhguy==%ld-----i==[%d]\n",tm,i);
		 mydebugTm.pre_tm=mydebugTm.cur_tm;
        img = zbar_video_next_image(proc->video);
		//if(img)
		//	zprintf(8,"sunzhguy....image %s\n",__func__);
		//zprintf(8,"image width=%d,Height=%d\n",img->format,img->format);//zbar_image_s
        if(img)
                              ...
                              int i)
{
    ...
    sunprintf(5, "kicking %d fds\n", state->polling.num);

    unsigned junk[2];
    int rc = read(state->kick_fds[0], junk, 2 * sizeof(unsigned));
	sunprintf(5, "state->kick_fds[0] %d junk=%d,%d\n", state->kick_fds[0],junk[0],junk[1]);
    ...
    if(proc->threaded) {
        ...
        if(pipe(state->kick_fds))
            return(err_capture(proc, SEV_FATAL, ZBAR_ERR_SYSTEM, __func__,
                               "failed to open pipe"));
		printf("state->kick_fds[%d],state->kick_fds[%d]\n",state->kick_fds[0],state->kick_fds[1]);
