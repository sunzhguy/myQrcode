...
zbar_video_t *zbar_video_create ()
{
    ...
    for(i = 0; i < ZBAR_VIDEO_IMAGES_MAX; i++) {
        zbar_image_t *img = vdo->images[i] = zbar_image_create();//zbar_image_s
        if(!img) {
            zbar_video_destroy(vdo);
            return(NULL);
        }
                     ...
                     const char *dev)
{
    ...
    video_lock(vdo);
    if(vdo->intf != VIDEO_INVALID) {//sunzhguy  vod----intf ===none v4l v4l2 vfw...
        if(vdo->cleanup) {
            vdo->cleanup(vdo);
            vdo->cleanup = NULL;
        }
                               ...
                               int iomode)
{
    ...
    vdo->iomode = iomode;
	printf("sunzhguy enter video_request_iomode==%d\n",iomode);
    return(0);
}
...
zbar_image_t *zbar_video_next_image (zbar_video_t *vdo)
{
    ...
    frame = vdo->frame++;//sunzhguy  to flow here to sea  
    img = vdo->dq(vdo);  //zbar_video_s
    sunprintf(9,":sunzhguy get frame=%d\n",frame);
    if(img) {
        img->seq = frame;
        if(vdo->num_images < 2) {
